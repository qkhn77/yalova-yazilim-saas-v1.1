<x-filament-panels::page>
    ...
    <div class="yk-info-card-grid grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div class="yk-info-card yk-account-kpi-card min-w-0 rounded-xl p-3 sm:p-4"><x-heroicon-o-banknotes class="yk-info-card-icon" /><div class="text-xs text-gray-500">Güncel bakiye</div><div class="mt-1 text-xl font-semibold tabular-nums">{{ $this->aktifBakiyeMetni($record) }}</div><div class="mt-1 text-xs text-gray-500">Aktif kasa hareketleri</div></div>
        <div class="yk-info-card yk-account-kpi-card min-w-0 rounded-xl p-3 sm:p-4"><x-heroicon-o-building-storefront class="yk-info-card-icon" /><div class="text-xs text-gray-500">Kasa hesabı</div><div class="mt-1 text-base font-semibold">{{ $record->ad }}</div><div class="mt-1 text-xs text-gray-500">Kod: {{ $record->kod ?: '—' }}</div></div>
        <div class="yk-info-card yk-account-kpi-card min-w-0 rounded-xl p-3 sm:p-4"><x-heroicon-o-check-badge class="yk-info-card-icon" /><div class="text-xs text-gray-500">Durum</div><div class="mt-1 text-base font-semibold">{{ $record->durum?->value === 'aktif' ? 'Aktif' : 'Pasif' }}</div><div class="mt-1 text-xs text-gray-500">Para birimi: {{ $record->para_birimi ?: '—' }}</div></div>
        <div class="yk-info-card yk-account-kpi-card min-w-0 rounded-xl p-3 sm:p-4"><x-heroicon-o-user class="yk-info-card-icon" /><div class="text-xs text-gray-500">Sorumlu</div><div class="mt-1 text-base font-semibold">{{ $record->sorumlu ?: 'Atanmamış' }}</div><div class="mt-1 text-xs text-gray-500">Hesap bilgileri ve hareket özeti</div></div>
    </div>
