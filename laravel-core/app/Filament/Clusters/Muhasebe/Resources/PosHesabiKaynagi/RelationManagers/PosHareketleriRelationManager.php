...
namespace App\Filament\Clusters\Muhasebe\Resources\PosHesabiKaynagi\RelationManagers;
...
class PosHareketleriRelationManager extends RelationManager
{
    use HasMovementDetails;
    protected static string $relationship = 'posHareketleri';

    protected static ?string $title = 'POS hareketleri';
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(fn (Builder $query) => $query
                ->select(['id', 'firma_id', 'pos_hesap_id', 'finans_hareket_id', 'tutar', 'para_birimi', 'brut_tutar', 'komisyon_tutari', 'slip_no', 'provizyon_no', 'durum'])
                ->with(['finansHareketi:id,firma_id,cari_id,tur,tarih,aciklama,referans_turu,referans_id,durum', 'finansHareketi.cari:id,firma_id,ad']))
            ->columns(array_merge([
