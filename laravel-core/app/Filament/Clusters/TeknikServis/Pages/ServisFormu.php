...
class ServisFormu extends Page
{
    ...
    protected static ?string $title = 'Servis Formu';

    protected static ?string $slug = 'belgeler/servis-formu';
    ...
    public function mount(): void
    {
        ...
        $this->sablon = $this->varsayilanSablonu($this->sablonTuru());
    }

    public function getHeading(): string|Htmlable
    {
        return $this->sablonTuru() === 'servis_fisi' ? 'Servis Fişi' : 'Servis Formu';
    }
    ...
    public function icerik(): Htmlable
    {
        ...
        if ($html === '') {
            return new HtmlString('<div class="text-sm text-gray-700">Servis formu şablonu bulunamadı.</div>');
        }
    ...
    public function sayfaCss(): string
    {
        ...
        $sablonKod = (string) ($this->sablon?->kod ?? '');
        ...
        $kenarBosluk = $sablonKod === 'teknik-servis-formu-a4'
            ? '0 10mm 10mm 10mm'
            : match ($sayfaTipi) {
                '58mm', '80mm' => '4mm',
                '10x10mm' => '0',
    ...
    protected function varsayilanSablonu(string $sablonTuru): ?TeknikServisBaskiSablonu
    {
        ...
        $servis->firmaSablonlariniHazirla($firmaId, $sablonTuru);
        ...
        foreach ($servis->hazirSablonlar($sablonTuru) as $hazirSablon) {
            if ((string) ($hazirSablon['kod'] ?? '') !== (string) $sablon->kod) {
                continue;
            }

            $sablon->forceFill([
                'ad' => (string) ($hazirSablon['ad'] ?? $sablon->ad),
                'sayfa_tipi' => (string) ($hazirSablon['sayfa_tipi'] ?? $sablon->sayfa_tipi),
                'sablon_html' => (string) ($hazirSablon['sablon_html'] ?? $sablon->sablon_html),
                'sablon_css' => (string) ($hazirSablon['sablon_css'] ?? $sablon->sablon_css),
            ]);
    ...
    private function sablonVerileri(): array
    {
        ...
        return [
            ...
            '{{TESLIM_TARIHI}}' => e(optional($this->kayit->teslim_tarihi)->format('d.m.Y H:i') ?? '-'),
            '{{TAHMINI_TESLIM}}' => e($this->tahminiTeslimMetni()),
            ...
            '{{CIHAZ_SIFRESI}}' => e($this->cihazSifresiMetni()),
            '{{AKSESUARLAR}}' => e($this->aksesuarMetni()),
            '{{FIZIKSEL_DURUM}}' => nl2br(e((string) ($this->kayit->musteriye_gorunen_not ?: '-'))),
            '{{CIHAZ_FOTOGRAFLARI}}' => $this->cihazFotograflariHtml(),
            '{{CIHAZ_FOTOGRAFLARI_BLOKU}}' => $this->cihazFotograflariBlokuHtml(),
            ...
            '{{TESLIM_NOTU_BLOKU}}' => $this->teslimNotuBloguHtml(),
            ...
            '{{PARCA_BILGISI}}' => e($this->parcaBilgisiMetni()),
            '{{MUSTERI_ONAY_DURUMU}}' => e($this->musteriOnayDurumuMetni()),
            '{{ONAY_NOTU}}' => nl2br(e($this->onayNotuMetni())),
            '{{IC_SERVIS_NOTU}}' => nl2br(e((string) ($this->kayit->ic_servis_notu ?: '-'))),
    ...
    private function teslimNotuBloguHtml(): string
    {
        $teslimNotu = trim((string) ($this->kayit?->teslim_notu ?? ''));

        if ($teslimNotu === '') {
            return '';
        }

        return '<div class="tsf-box tsf-full">'
            .'<div class="tsf-box-title">Teslim Notu</div>'
            .'<div>'.nl2br(e($teslimNotu)).'</div>'
            .'</div>';
    }
    ...
    private function aksesuarMetni(): string
    {
        ...
        return is_array($adlar) && $adlar !== [] ? implode(', ', $adlar) : '-';
    }

    private function tahminiTeslimMetni(): string
    {
        return optional($this->kayit?->teslim_tarihi)->format('d.m.Y H:i') ?? '—';
    }

    private function cihazSifresiMetni(): string
    {
        return '—';
    }

    private function parcaBilgisiMetni(): string
    {
        $parcalar = $this->kayit?->kalemler
            ?->map(function ($kalem): string {
                ...
                $miktarMetni = $miktar > 0 ? ' x'.$this->miktarFormatla($miktar) : '';

                return $ad.$miktarMetni;
            })
            ->filter()
            ->unique()
            ->values()
        ...
        return is_array($parcalar) && $parcalar !== [] ? implode(', ', $parcalar) : 'Parça veya hizmet kaydı yok.';
    }
    ...
    private function cihazFotograflariHtml(): string
    {
        ...
        $items = $gorseller->map(function (string $path): string {
            ...
            return '<div class="sk-photo-item"><img src="'.e($url).'" alt="Cihaz fotoğrafı"></div>';
        })->filter()->implode('');
    ...
    private function cihazFotograflariBlokuHtml(): string
    {
        $icerik = $this->cihazFotograflariHtml();
        if ($icerik === '') {
            return '';
        }

        return '<div class="photo-record-block">'
            .'<div class="photo-record-title">Gorsel Kayit</div>'
            .$icerik
            .'</div>';
    }
