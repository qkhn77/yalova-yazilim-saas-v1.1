...
final class Code128SvgUretici
{
    /** @var array<int, string> */
    private const DESENLER = [
    ...
    {
        $deger = trim($hamDeger);
        if ($deger === '' || preg_match('/[^\x20-\x7E]/', $deger)) {
            return null;
        }
        ...
        $etiket = htmlspecialchars($deger, ENT_QUOTES, 'UTF-8');

        $svg = '<svg xmlns="http://www.w3.org/2000/svg" width="'.$cizimGenisligi.'" height="'.$yukseklik.'" viewBox="0 0 '.$cizimGenisligi.' '.$yukseklik.'" role="img" aria-label="Code128 '.$etiket.'">';
        ...
        $svg .= '<text x="'.(int) floor($cizimGenisligi / 2).'" y="'.($barYukseklik + 11).'" text-anchor="middle" font-size="11" font-family="Arial, sans-serif" fill="#111111">'.$etiket.'</text>';
        $svg .= '</svg>';
