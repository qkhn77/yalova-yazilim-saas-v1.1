...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('project_categories', function (Blueprint $table) {
            if (! Schema::hasColumn('project_categories', 'meta_title')) {
                $table->string('meta_title', 100)->nullable()->after('name');
            }
            if (! Schema::hasColumn('project_categories', 'meta_description')) {
                $table->string('meta_description', 165)->nullable()->after('description');
            }
            if (! Schema::hasColumn('project_categories', 'meta_keywords')) {
